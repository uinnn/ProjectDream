...
class CompoundTag : MapTag<String, Tag>, java.io.Serializable {
  ...
  fun item(key: String, default: ItemStack = EmptyItemStack): ItemStack {
    return compoundOrNull(key)?.createItem() ?: return default
  }

  fun itemOrNull(key: String): ItemStack? {
    return compoundOrNull(key)?.createItem()
  }
  ...
  operator fun set(key: String, value: ItemStack) = put(key, value.store())
...
/**
 * Creates a new [CompoundTag] storing [storable] inside.
 */
...
/**
 * A immutable global compound tag.
 *
 * Similar to [GlobalCompound] but this is immutable.
 */
object ImmutableGlobalCompound : CompoundTag() {
  override fun put(key: String, value: Tag) = EmptyTag
  override fun putAll(from: Map<out String, Tag>) {}
  override fun get(key: String) = EmptyTag
