...
class PlayerList(val server: Server) : Tickable, PlayerStorage {
  ...
  override val playersNames get() = usernames
  ...
  fun sendPacketToAllNear(packet: HandledPacket, x: Double, y: Double, z: Double, radius: Double, level: Level) {
    forEachPlayer { player ->
      if (player.level != level && !player.isNear(x, y, z, radius)) return@forEachPlayer
  ...
  fun sendPacketToAllNearExcept(
    ...
    if (player == except && player.level != level && !player.isNear(x, y, z, radius)) return@forEachPlayer
    player.sendPacket(packet)
  }
   ...
   */
  fun sendChat(message: Component, isChat: Boolean = true) {
