...
class Item : Keyable, Locale {
   ...
   */
  val block: Block? get() = if (this is ItemBlock) block else null
   ...
   */
  fun getAttributes(item: ItemStack): Multimap<String, IAttribute> = HashMultimap.create()
  ...
  fun onRightClick(level: Level, item: ItemStack, player: Player): ItemStack {
    // todo
    if (this is Containerable) {
      val container = createContainer(player, player.inventory)
   ...
   */
  fun mine(level: Level, item: ItemStack, player: Player, block: Block, pos: Pos): Boolean {
    return true
  }
