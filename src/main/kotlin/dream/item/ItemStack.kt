...
data class ItemStack(
   ...
   */
  infix fun shrinkIsEmpty(quantity: Int): Boolean {
    amount -= quantity
    return isEmpty
  }
   ...
   */
  final inline fun ifAir(action: ItemStack.() -> Unit) {
    if (isAir) {
      action(this)
    }
  }
   ...
   */
  final inline fun ifNotAir(action: ItemStack.() -> Unit) {
    if (!isAir) {
      action(this)
    }
  }
   ...
   */
  final inline fun <reified T : Item> ifIs(action: ItemStack.() -> Unit) {
    if (item is T) {
      action(this)
    }
  }
   ...
   */
  final inline fun <reified T : Item> ifNotIs(action: ItemStack.() -> Unit) {
    if (item !is T) {
      action(this)
    }
  }
   ...
   */
  final inline fun withTag(builder: CompoundTag.() -> Unit): ItemStack {
    tag.apply(builder)
    return this
  }
   ...
   * @param checkTag Whether to check the NBT tag of the ItemStack (default: true).
   * @param checkAmount Whether to check the amount of items in the ItemStack (default: true).
   * @return `true` if the ItemStacks are equal, `false` otherwise.
   */
  ...
  override fun load(tag: CompoundTag) {
    ...
    metadata = tag.short("Damage").toInt().coerceAtLeast(0)
 ...
 */
inline fun stackOf() = EmptyItemStack

/**
 * Returns [EmptyItemStack]
 */
inline fun emptyStack() = EmptyItemStack
