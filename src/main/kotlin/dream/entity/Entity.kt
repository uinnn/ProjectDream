...
abstract class Entity : Identifiable, Localizable, CompoundStorable, Tickable, Nameable, Scope, Comparable<Entity> {
   ...
   /**
    * Gets if the level of this entity is clientside
    */
    ...
    */
   var motion: Pos
      get() = Pos(motionX, motionY, motionZ)
      set(value) {
         motionX = value.x
         motionY = value.y
         motionZ = value.z
   ...
   val isBurning: Boolean
      get() = !isFireImmune && (fireTicks > 0 || isClient && isBurningFlag)
    ...
    */
   fun setFlag(flag: Int, set: Boolean) {
   ...
   fun setSize(width: Float, height: Float) {
      if (this.width != width || this.height != height) {
         ...
         if (width > old && !isFirstUpdate && !isClient) {
    ...
    *
    * This is sightly different from
    * ,
    * `damage(damage: Damage, amount: Float)`
    *
    * cause this damage the entity fully directly without caring
    * of [isInvulnerable] and [Damage] characteristics,
    * resuming, this will subtract their health without side-effects.
    */
   fun damageIgnore(amount: Float) {
   ...
   fun isInvulnerable(damage: Damage, amount: Float = 0f): Boolean {
      return isInvulnerable
         && damage !is VoidDamage
         && !damage.damageInCreative
         && !damage.canDamage(this, amount)
   }
   ...
   fun onEnchantmentDamage(ench: EnchantmentDamage, attacker: Entity, item: ItemStack, level: Int): Boolean {
      damageIgnore(ench.getDamage(attacker, this, item, level))
      return true
   }
