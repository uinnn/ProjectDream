...
class BlockBookshelf : Block(Materials.WOOD, CreativeTab.BLOCKS) {
  ...
  override fun onRemoved(level: Level, pos: Pos, state: IState) {
    getDrop(level, pos, state).drop(level, pos)
  }
  
  override fun onInteract(level: Level, pos: Pos, state: IState, player: Player, side: Direction, hit: Pos, click: Click) {
    if (click.isRight) {
