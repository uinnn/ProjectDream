 ...
 */
abstract class BaseChunkMap<T : Any>(
  override val size: ChunkSize,
  override var data: ByteArray = ByteArray(size.volume) { -1 }
) : ChunkMap<T> {

  /**
   * Sets the data of this chunk map to the given array.
   */
  override fun mutate(data: ByteArray) {
    if (data.size != this.data.size) {
      throw IllegalArgumentException("Invalid data size: ${data.size} not equals to ${this.data.size}")
    }
    this.data = data
  }
