...
public enum class Direction(
   ...
   companion object {
      ...
      @JvmField val horizontal = listOf(SOUTH, WEST, NORTH, EAST)
      @JvmField val lookup = values.associateBy { it.id }
      
      /**
       * Gets a direction by index.
       */
      fun front(index: Int) = values[abs(index % 6)]
      ...
      fun byAxis(axis: Axis, direction: AxisDirection): Direction {
         return values.find { it.axis == axis && it.direction === direction } ?: NORTH
      }
      
      /**
       * Gets the nearest direction by pos.
       */
      fun nearest(point: Pos) = nearest(point.x, point.y, point.z)
      ...
      fun nearest(x: Double, y: Double, z: Double): Direction {
         return values.maxByOrNull { (x * it.stepX) + (y * it.stepY) + (z * it.stepZ) } ?: NORTH
      }
   }
}
